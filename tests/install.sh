...
printf '%s\n' '#!/bin/sh' \
    ...
    '    printf "%s\n" "https://github.com/pulkitxm/pukbot/releases/tag/v0.3.0"' \
    '    exit 0' \
    'fi' \
    '[ -n "${destination}" ]' \
    'case "${url}" in' \
    '    */SHA256SUMS) cp "$PUKBOT_TEST_FIXTURES/SHA256SUMS" "${destination}" ;;' \
    '    *) cp "$PUKBOT_TEST_FIXTURES/$PUKBOT_TEST_ASSET" "${destination}" ;;' \
    'esac' >"${FAKE_BIN}/curl"
