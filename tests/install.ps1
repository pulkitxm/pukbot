...
$TestRoot = Join-Path ([IO.Path]::GetTempPath()) ("gitbot-installer-tests-" + [Guid]::NewGuid().ToString("N"))
...
function global:Invoke-WebRequest {
    ...
    Add-Content -LiteralPath $global:GitbotDownloadsLog -Value $Uri
    $asset = [IO.Path]::GetFileName(([Uri] $Uri).AbsolutePath)
    Copy-Item -LiteralPath (Join-Path $global:GitbotFixtures $asset) -Destination $OutFile
}

$global:GitbotFixtures = $Fixtures
$global:GitbotDownloadsLog = $DownloadsLog
...
try {
    ...
    Remove-Variable GitbotFixtures -Scope Global -ErrorAction SilentlyContinue
    Remove-Variable GitbotDownloadsLog -Scope Global -ErrorAction SilentlyContinue
